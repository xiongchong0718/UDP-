/*!
 @file AsyncUdpSocket.m
 @brief This is the file where some socket methods come true.
 @author vigek
...
#if TARGET_OS_IPHONE
/*! Note: You need to add the CFNetwork Framework in this project */
...
/*! @enum AsyncUdpSocketFlags
    @brief This enum contains some flags about socket.
    @field kDidBind  If set, bind has been called.
    @field kDidConnect If set, connect has been called.
    @field kSock4CanAcceptBytes If set, we know socket4 can accept bytes. If unset, it's unknown.
    @field kSock6CanAcceptBytes If set, we know socket6 can accept bytes. If unset, it's unknown.
    @field kSock4HasBytesAvailable If set, we know socket4 has bytes available. If unset, it's unknown.
    @field kSock6HasBytesAvailable If set, we know socket6 has bytes available. If unset, it's unknown.
    @field kForbidSendReceive  If set, no new send or receive operations are allowed to be queued.
    @field kCloseAfterSends If set, close as soon as no more sends are queued.
    @field kCloseAfterReceives If set, close as soon as no more receives are queued.
    @field kDidClose If set, the socket has been closed, and should not be used anymore.
    @field kDequeueSendScheduled If set, a maybeDequeueSend operation is already scheduled.
    @field kDequeueReceiveScheduled If set, a maybeDequeueReceive operation is already scheduled.
    @field kFlipFlop Used to alternate between IPv4 and IPv6 sockets.
 */
enum AsyncUdpSocketFlags
{
    ...
    kSock4CanAcceptBytes     = 1 <<  2,
    kSock6CanAcceptBytes     = 1 <<  3,
 
	kSock4HasBytesAvailable  = 1 <<  4,
	kSock6HasBytesAvailable  = 1 <<  5,
	kForbidSendReceive       = 1 <<  6,
	kCloseAfterSends         = 1 <<  7,
	kCloseAfterReceives      = 1 <<  8,
	kDidClose                = 1 <<  9,
	kDequeueSendScheduled    = 1 << 10,
	kDequeueReceiveScheduled = 1 << 11,
	kFlipFlop                = 1 << 12,
};
...
- (id)initWithDelegate:(id)delegate userData:(long)userData enableIPv4:(BOOL)enableIPv4 enableIPv6:(BOOL)enableIPv6
{
	if((self = [super init]))
	{
		...
		/*! Disable continuous callbacks for read and write.
		    If we don't do this, the socket(s) will just sit there firing read callbacks at us hundreds of times a second if we don't immediately read the available data.
         */
		if(theSocket4)
		{
...
/*!
 @brief Attempts to convert the given host/port into and IPv4 and/or IPv6 data structure.
 @discussion The data structure is of type sockaddr_in for IPv4 and sockaddr_in6 for IPv6.
				 ...
				 address6:(NSData **)address6
{
	...
	{
        /*! tip:getaddrinfo("localhost",...) fails on 10.5.3 */
...
/*!
 @brief  Binds the underlying socket(s) to the given port.
 @discussion The socket(s) will be able to receive data on any interface.
...
/*!
 @brief Binds the underlying socket(s) to the given address and port.
 @discusiion The sockets(s) will be able to receive data only on the given interface.
...
- (BOOL)bindToAddress:(NSString *)host port:(UInt16)port error:(NSError **)errPtr
{
	...
	if(address6)
	{
		/*! Tip: The iPhone doesn't currently support IPv6 */
...
/*!
 @brief Connects the underlying UDP socket to the given host and port.
 @discussion If an IPv4 address is resolved, the IPv4 socket is connected, and the IPv6 socket is invalidated and released.
...
/*!
 @brief Connects the underlying UDP socket to the remote address.
 @discussion If the address is an IPv4 address, the IPv4 socket is connected, and the IPv6 socket is invalidated and released.
...
/*!
 ...
 @discussion Group should be a multicast IP address (eg. @"239.255.250.250" for IPv4).
...
/*!
 @brief Called when the socket has received the requested datagram.
 @discussion Due to the nature of UDP, you may occasionally receive undesired packets.
             These may be rogue UDP packets from unknown hosts,or they may be delayed packets arriving after retransmissions have already occurred.
             ...
             If you ever need to ignore a received packet, simply return NO,and AsyncUdpSocket will continue as if the packet never arrived.
             ...
             If the expected data then arrives within the next 250 milliseconds,this delegate method will be invoked, with a tag of 15, just as if the rogue data never appeared.
...
/*!
 @brief Returns a standard error object for the current errno value.
...
/*!
 @brief Returns a standard error message for a CFSocket error.
...
- (NSString *)connectedHost:(CFSocketRef)theSocket
{
	...
	/*! So if this is called prior to binding/connecting/sending, it may not be updated again when necessary,and will continue to return the old value of the socket peer address. */
...
/*!
 @brief This method is used to send message.
 @param data 发送的内容
 @param host 目标ip
 @param port 端口
 @param timeout 超时时间
...
/*!
 @brief Puts a maybeDequeueSend on the run loop.
 */
- (void)scheduleDequeueSend
{
...
/*!
 @brief This method starts a new send, if needed.
 @discussion It is called when a user requests a send.
*/
- (void)maybeDequeueSend
{
...
/*!
 @brief This method is called when a new read is taken from the read queue or when new data becomes available on the stream.
*/
...
- (void)completeCurrentSend
{
    ...
    if (theCurrentSend != nil) [self endCurrentSend];
}
...
/*!
 @brief  Ends the current send, and all associated variables such as the send timer.
*/
- (void)endCurrentSend
{
...
/*!
 @brief Puts a maybeDequeueReceive on the run loop.
*/
- (void)scheduleDequeueReceive
{
...
/*!
 @brief Starts a new receive operation if needed
*/
- (void)maybeDequeueReceive
{
	/*! Unset the flag indicating a call to this method is scheduled */
	...
	if (theCurrentReceive == nil)
	{
		...
		if ([theReceiveQueue count] > 0)
		{
			...
			/*! We also check them in alternating orders to prevent starvation if both of them have a continuous flow of incoming data.
             */
...
- (BOOL)maybeCompleteCurrentReceive
{
	...
	{
        /*! Caller may have disconnected. */
        if (theCurrentReceive != nil) [self endCurrentReceive];
	}
	return finished;
}
...
{
    /*! Old timer. Ignore it. */
	if (timer != theReceiveTimer) return;
	if (theCurrentReceive != nil)
	{
...
/*!
 @brief  This is the callback we setup for CFSocket.
 */
